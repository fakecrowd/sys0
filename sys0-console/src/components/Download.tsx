...
import { DOWNLOAD_SECTIONS } from "../downloadNav";
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        {data && (
          <>
            ...
            {DOWNLOAD_SECTIONS.map((section, index) => (
              <div key={section.kind} style={index ? { marginTop: 24 } : undefined}>
                <AssetTable
                  title={section.title}
                  empty={section.empty}
                  assets={data.assets.filter((a) => section.kind === "rescue" ? a.kind === "rescue" : a.kind !== "rescue")}
                />
              </div>
            ))}

            <div className="panel p-4 mt-6 mono-sm" style={{ lineHeight: 1.7 }}>
              <div style={{ color: "var(--accent)", marginBottom: 6 }}>快速开始</div>
              <div style={{ opacity: 0.85 }}>
                下载后直接<strong>运行</strong>即可，客户端会自动连接到{" "}
                <code>{location.host}</code>。
              </div>
              <div style={{ opacity: 0.85, marginTop: 6 }}>
                Download and run the client to connect to{" "}
                <code>{location.host}</code>.
                <br />
